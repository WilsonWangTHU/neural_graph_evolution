...
class optimization_agent(base_agent):
    ...
    def update_ppo_parameters(self, paths):
        ...
        feed_dict, nervenetplus_batch_pos = \
            self.prepared_network_feeddict(paths)
        ...
        for i_epochs in range(self.args.optim_epochs +
            ...
            if self.args.nervenetplus:
                minibatch_id_candidate = list(
                    range(len(nervenetplus_batch_pos))
                )
    ...
    def prepared_network_feeddict(self, data_dict):
        ...
        feed_dict, nervenetplus_batch_pos = \
            self.prepared_policy_network_feeddict(obs_n, data_dict)
        ...
        if self.args.nervenetplus:
            ...
            for node_type in self.node_info['node_type_dict']:
                feed_dict[self._input_hidden_state[node_type]] = \
                    np.concatenate([path['hidden_state'][node_type] for path in data_dict])
    ...
    def prepare_feed_dict_map(self):
        ...
        if self.args.use_gnn_as_policy:
            ...
            if self.args.nervenetplus:
                assert self.args.optim_batch_size % \
