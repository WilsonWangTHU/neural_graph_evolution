...
class simple_GRU(object):
    ...
    def __init__(self,
                 ...
                 init_method, wd=None,
                 dtype=tf.float32, init_std=None,
                 scope="SIM_GRU"):
        ...
        with tf.variable_scope(scope):
            ...
            self._w_x = weight_variable(
                [input_dim, hidden_dim * 3], init_method=self._init_method,
                init_para={"mean": 0.0, "stddev": init_std},
                wd=wd, name="w_xi", dtype=dtype
            )
            self._w_h = weight_variable(
                [hidden_dim, hidden_dim * 3], init_method=self._init_method,
                init_para={"mean": 0.0, "stddev": init_std},
                wd=wd, name="w_hi", dtype=dtype
            )
            self._b = weight_variable(
                [hidden_dim * 3], init_method="constant",
                init_para={"val": 0.0}, wd=wd, name="b_i", dtype=dtype
            )

    def __call__(self, x, state, is_batch_mul=False):

        z = tf.matmul(x, self._w_x) + tf.matmul(state, self._w_h) + self._b
        g_i, g_r, u = tf.split(axis=1, num_or_size_splits=3, value=z)
