...
import argparse
from .evolutionary_config import get_evolutionary_config
import init_path


def get_config(evolution=False):
    # get the parameters
    parser = argparse.ArgumentParser(description='graph_rl.')

    # nervenet++ options
    parser.add_argument("--nervenetplus", type=int, default='1',
                        help='whether to use nervenetplusplus')

    parser.add_argument("--use_nervenet", type=int, default='1',
                        help='whether to use nervenetplusplus')
    ...
    # dm_control package cmd options
    # domain and task_name are integrated in task
    parser.add_argument('--dm', action='store_true', default=False,
                        help='Use dm_control package instead of gym.')
    parser.add_argument('--dm_render', action='store_true', default=False,
                        help='Render the simulation (supposingly only at test time)')
    ...
    if evolution:
        parser = get_evolutionary_config(parser)

    # visualization debug
    parser.add_argument('--viz', action='store_true',
                        help='Use visdom to visualize the training curves')
    parser.add_argument('--test_env', action='store_true',
                        help='See ../env/test_env_wraper')
...
def get_ggnn_config(parser):
    ...
    parser.add_argument("--gnn_output_per_node", type=int, default=3)
    ...
    parser.add_argument("--gnn_output_option", type=str, default='unified',
                        ...
                        ''')
    parser.add_argument("--gnn_embedding_option", type=str, default='parameter',
                        help='''
                        @Options:
